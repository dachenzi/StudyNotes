...
### 4.4.2 类方法
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;可以在外部直接使用类名.方法名来调用的方法称为`类方法`，类方法属于一种动态加载的概念，需要使用classmethod装饰器来进行处理，才能直接在外部使用类名调用
```python
In [31]: class Person: 
    ...:     @classmethod 
...
>类方法类似于C++、Java中的静态方法。
        ...
        类方法(classmethod)：
            必须要有一个位置参数cls
            通过实例访问类方法，实际上是由@classmethod获取实例的类，然后传入的
            在类方法中是无法获取到self，这种实例对象的。
            > 可以通过实例来操作类，但是没办法通过类来操作实例
    
        静态方法(staticmethod)：(用的极少)
            在调用时不会隐式的传入参数
            静态方法，只表明这个方法属于这个名称空间，函数归在一起，方便组织管理。
            可以理解为一个函数封装，可以通过类调用、也可以通过实例调用，只是强制的放在类中而已。
