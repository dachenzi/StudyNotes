...
- [1 魔术方法](#1-魔术方法)
- [2 类的魔术方法](#2-类的魔术方法)
    - [2.1 基本的魔法方法和常用属性](#21-基本的魔法方法和常用属性)
    - [2.2 有关属性](#22-有关属性)
    - [2.3 比较操作符](#23-比较操作符)
    - [2.4 算数运算符](#24-算数运算符)
    - [2.5 反运算](#25-反运算)
    - [2.6 增量赋值运算](#26-增量赋值运算)
    - [2.7 一元操作符](#27-一元操作符)
    - [2.8 类型转换](#28-类型转换)
    - [2.9 上下文管理（with 语句）](#29-上下文管理with-语句)
    - [2.10 容器类型](#210-容器类型)
- [3 常用方法](#3-常用方法)
    - [3.1 查看属性](#31-查看属性)
    - [3.2 实例化](#32-实例化)
    - [3.3 hash相关](#33-hash相关)
        - [3.3.1 hash相同能否去重](#331-hash相同能否去重)
        - [3.3.2 比较内容是否相同](#332-比较内容是否相同)
        - [3.3.3 坐标轴小例子](#333-坐标轴小例子)
    - [3.4 bool类型](#34-bool类型)
    - [3.5 可视化](#35-可视化)
    - [3.6 运算符重载](#36-运算符重载)
        - [3.6.1 实现两个实例相减](#361-实现两个实例相减)
        - [3.6.2 坐标轴小例子](#362-坐标轴小例子)
        - [3.6.3 应用场景](#363-应用场景)
        - [3.6.4 total_ordering装饰器](#364-total_ordering装饰器)
    - [3.7 容器相关方法](#37-容器相关方法)
    - [3.8 可调用对象](#38-可调用对象)
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当dir(obj)时，obj的\_\_dir\_\_()方法被调用，如果当前实例不存在该方法，则按照mro开始查找，如果父类都没有定义，那么最终会找到object.\_\_dir\_\_()方法，该方法会最大程度的收集属性信息。
```python
class A:
    def __dir__(self):
        return 'ab'
...
即：使用bool时，先判断\_\_bool\_\_，然后再判断\_\_len\_\_，否则True
```python
class A:
